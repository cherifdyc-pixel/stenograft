          ...
          {conversations.map((conv: any) => {
            ...
            return (
              <Link key={conv.id} href={`/dashboard/messages/${conv.id}`} style={{ textDecoration: 'none' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', background: '#0a0a0a', borderRadius: '12px', border: '1px solid #111' }}>
                  <div style={{ width: '48px', height: '48px', borderRadius: '50%', background: '#E0492F', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '16px', fontWeight: 700, color: '#fff', flexShrink: 0 }}>
                    {initiales}
                  </div>
                  <div style={{ flex: 1 }}>
